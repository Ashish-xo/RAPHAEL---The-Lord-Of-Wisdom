...
public partial class MainPanel
{
    ...
    // ============================ PLAYER: [redacted] (client-side) ============================

    // ---- [redacted] category filter ([redacted] tab) ----
    private static readonly ObjCategory[] [redacted] =
    {
        ObjCategory.Ore, ObjCategory.Wood, ObjCategory.Grass, ObjCategory.Plant, ObjCategory.Flower,
        ObjCategory.Grave, ObjCategory.Tech, ObjCategory.ResourceOther, ObjCategory.Container, ObjCategory.Prison,
        ObjCategory.Castle, ObjCategory.Other,
    };
    private readonly System.Collections.Generic.List<(ObjCategory cat, ToggleRef toggle)> [redacted] = new();

    private void [redacted](GameObject parent, ObjCategory cat)
    {
        var row = MakeFaustRow(parent, $"[redacted]{(int)cat}");
        var t = UIFactory.CreateToggle(row, $"[redacted]{(int)cat}");
        UIFactory.SetLayoutElement(t.GameObject, minWidth: 360, preferredWidth: 400, flexibleWidth: 1, minHeight: 24, preferredHeight: 26, flexibleHeight: 0);
        t.Text.text = [redacted].Label(cat);
        t.Text.fontSize = Theme.ScaledUI(12);
        t.Text.alignment = TextAlignmentOptions.MidlineLeft;
        UIFactory.SetLayoutElement(t.Text.gameObject, minWidth: 320, preferredWidth: 360, flexibleWidth: 1, minHeight: 22, preferredHeight: 24, flexibleHeight: 0);
        t.Toggle.isOn = [redacted].IsShown(cat);
        TooltipHover.Attach(t.GameObject, $"Show {[redacted].Label(cat)} in the [redacted] and the [redacted] list.");
        t.OnValueChanged += v =>
        {
            int mask = Config.Settings.[redacted];
            if (v) mask |= (int)cat; else mask &= ~(int)cat;
            Config.Settings.Set[redacted](mask);
        };
        [redacted].Add((cat, t));
    }

    // Drive every category toggle to match a preset mask; each toggle's own handler folds the bit into the
    // setting (so the persisted mask and the visible toggles stay in lockstep without a tab rebuild).
    private void [redacted](ObjCategory mask)
    {
        foreach (var (cat, toggle) in [redacted])
        {
            bool on = (mask & cat) != 0;
            if (toggle?.Toggle != null && toggle.Toggle.isOn != on) toggle.Toggle.isOn = on;
        }
        // Categories not represented by a toggle (none today) would be lost; the represented set == AllResources|Container|Prison.
    }

    private static readonly float[] [redacted]Radii = { 25f, 50f, 100f };
    private int [redacted] = 1;     // 50 m default
    private ButtonRef [redacted];
    private TextMeshProUGUI [redacted];
    private GameObject [redacted];

    private float [redacted]Radius() => [redacted]Radii[Mathf.Clamp([redacted], 0, [redacted]Radii.Length - 1)];

    private static string [redacted]ToggleLabel()
        => (Plugin.UIManager?.IsOverlayOpen(PanelType.[redacted]) ?? false) ? "Hide [redacted]" : "Show [redacted]";

    private static string [redacted]()
        => [redacted].IsOn ? "Hide [redacted]" : "Show [redacted]";

    private void Build[redacted](GameObject page)
    {
        EnsureFaustSubscribed();

        var intro = AddCard(page, "[redacted]Intro");
        AddSectionHeading(intro, "[redacted]s");
        AddBodyText(intro,
            "A <b>client-side</b> scan of objects around you — resource nodes, storage containers / stations, " +
            "prison cells, and castle pieces within range — read straight off what the game has streamed to " +
            "your client (no server query, no toll). Use the <b>[redacted]</b> below to scan for exactly " +
            "what you want; the same filter drives the [redacted] and the [redacted].");

        var overlayCard = AddCard(page, "[redacted]OverlayCard");
        AddSectionHeading(overlayCard, "On-screen overlays");
        AddBodyText(overlayCard,
            "<b>[redacted]</b> float a name tag <b>over each [redacted]'s actual position</b> (the " +
            "tag tracks the object as you move). The <b>[redacted]</b> is a draggable corner panel listing " +
            "what's around you. Both are fully client-side (no server query, no toll). If the labels can't " +
            "find a camera or the object set is empty they simply don't draw.");
        var ovRow = MakeFaustRow(overlayCard, "[redacted]OverlayRow");
        ButtonRef worldBtn = null;
        worldBtn = AddFaustButton(ovRow, "[redacted]", [redacted](),
            "Show/hide floating name tags drawn over each [redacted] in the world (resources, containers, prisons). Client-side.",
            () => { [redacted].Toggle(); SetFaustButtonText(worldBtn, [redacted]()); }, 190);
        ButtonRef lookBtn = null;
        lookBtn = AddFaustButton(ovRow, "[redacted]Toggle", [redacted]ToggleLabel(),
            "Show/hide the draggable corner [redacted] (what your cursor is over + a [redacted]s list). Fully client-side.",
            () => { Plugin.UIManager?.ToggleOverlay(PanelType.[redacted]); SetFaustButtonText(lookBtn, [redacted]ToggleLabel()); }, 190);

        // [redacted] reach — how far labels appear. Default 25m; raise it so objects further away still tag.
        var radRow = MakeFaustRow(overlayCard, "[redacted]Row");
        ButtonRef radBtn = null;
        radBtn = AddFaustButton(radRow, "[redacted]Cycle", $"[redacted]: {Config.Settings.[redacted]}m",
            "How far the [redacted] reach — click to cycle 15 → 25 → 40 → 60 → 80 m. An object only labels if it's within this distance AND on-screen.",
            () =>
            {
                int[] opts = { 15, 25, 40, 60, 80 };
                int idx = System.Array.IndexOf(opts, Config.Settings.[redacted]);
                int next = opts[(idx + 1) % opts.Length];   // idx == -1 (custom value) → opts[0]
                Config.Settings.Set[redacted](next);
                SetFaustButtonText(radBtn, $"[redacted]: {next}m");
            }, 170);

        var filterCard = AddCard(page, "[redacted]FilterCard");
        AddSectionHeading(filterCard, "[redacted] — what to show");
        AddBodyText(filterCard,
            "Pick exactly which categories of object you want to see. This one filter drives <b>all three</b>: " +
            "the <b>Scan</b> list below, the floating <b>[redacted]</b>, and the <b>[redacted]</b>. " +
            "Untick a category to hide it — e.g. ore + trees only while mining, or just containers while " +
            "raiding. <b>[redacted]</b> (floors/walls/pillars) are their own category and off by " +
            "default, so they no longer clutter the resource view. Classification is by prefab name " +
            "(best-effort) — if something lands in the wrong bucket, turn on diagnostics, note its name, and " +
            "I'll fix the chart.");
        [redacted].Clear();
        foreach (var cat in [redacted])
            [redacted](filterCard, cat);
        var quickRow = MakeFaustRow(filterCard, "[redacted]");
        AddFaustButton(quickRow, "[redacted]", "All", "Show every category.",
            () => [redacted](ObjCategory.All), 70);
        AddFaustButton(quickRow, "[redacted]", "Resources only",
            "Show only resource nodes (ore, wood, grass, plants, flowers, graves, tech); hide containers & prisons.",
            () => [redacted](ObjCategory.AllResources), 150);
        AddFaustButton(quickRow, "[redacted]", "None", "Hide every category (labels off).",
            () => [redacted](ObjCategory.None), 70);

        var controls = AddCard(page, "[redacted]Controls");
        var row = MakeFaustRow(controls, "[redacted]Btns");
        AddFaustButton(row, "[redacted]Scan", "Scan",
            "Scan for [redacted] within the selected radius (client-side; no server query).",
            Do[redacted]Scan, 90);
        [redacted] = AddFaustButton(row, "[redacted]Radius", $"Radius: {[redacted]Radius():0}m",
            "Click to cycle the scan radius.",
            () => { [redacted] = ([redacted] + 1) % [redacted]Radii.Length; SetFaustButtonText([redacted], $"Radius: {[redacted]Radius():0}m"); }, 130);
        AddBodyText(controls,
            $"<color={Theme.MutedBodyHex}>Scan results are filtered by the <b>[redacted]</b> above — only the " +
            "categories you've ticked are listed, with each object's exact category shown.</color>");
        [redacted] = AddBodyText(controls, "—", Theme.ScaledUI(11));

        var list = AddCard(page, "[redacted]ListCard");
        AddSectionHeading(list, "Results");
        [redacted] = MakeFaustListContainer(list, "[redacted]Rows");
        if ([redacted] != null)
            [redacted].text = $"<color={Theme.MutedBodyHex}>Click Scan to list objects within {[redacted]Radius():0}m.</color>";
    }

    private void Do[redacted]Scan()
    {
        if ([redacted] == null) return;
        ClearChildren([redacted]);

        if (!Services.Faust.[redacted].Available)
        {
            if ([redacted] != null)
                [redacted].text = "<color=#FFB070>[redacted]ning was disabled this session after repeated errors. Rejoin to retry.</color>";
            return;
        }

        // Scan, classify each hit into its EXACT category, then show only the categories the user ticked in
        // the [redacted]. "Uncategorized" (cameras, the player User entity, unresolved prefab GUIDs, and
        // anything the name heuristic doesn't recognize) is its own toggle, OFF by default — so it never
        // floods the list. The unmatched prefabs are only even *collected* when that toggle is on (or
        // diagnostics is on for name-identification), keeping the distance-capped result set focused on real
        // objects. Diagnostics still logs every nearby prefab name to LogOutput.log regardless.
        bool diag = Services.Faust.FaustDiag.Enabled;
        bool wantOther = [redacted].IsShown(ObjCategory.Other) || diag;
        var hits = Services.Faust.[redacted].ScanNearby([redacted]Radius(), includeResources: true, includeContainers: true, diagShowAll: wantOther);

        var rows = new System.Collections.Generic.List<(Services.Faust.[redacted].NearbyObject o, ObjCategory cat)>();
        foreach (var o in hits)
        {
            var cat = [redacted].Classify(o.Name, o.Kind);
            if ([redacted].IsShown(cat)) rows.Add((o, cat));
        }

        if ([redacted] != null)
        {
            int hidden = hits.Count - rows.Count;
            string hiddenNote = hidden > 0 ? $" <color={Theme.MutedBodyHex}>({hidden} hidden by your filter)</color>" : "";
            string diagNote = diag ? $" <color=#9FD0FF>(diagnostics on — every nearby prefab name is in LogOutput.log; tick “Uncategorized” to see unmatched ones here)</color>" : "";
            [redacted].text = (rows.Count == 0
                ? $"<color=#FFD75A>Nothing matching your filter within {[redacted]Radius():0}m.</color>"
                : $"<color=#90EE90>{rows.Count} object(s) within {[redacted]Radius():0}m.</color>" + hiddenNote) + diagNote;
        }

        if (rows.Count == 0) return;
        var cols = new[]
        {
            new FaustCol(Theme.ScaledWidth(170), 1, TextAlignmentOptions.MidlineLeft),  // Object
            new FaustCol(Theme.ScaledWidth(110), 0, TextAlignmentOptions.MidlineLeft),  // Category
            new FaustCol(Theme.ScaledWidth(60), 0, TextAlignmentOptions.MidlineRight),  // Distance
        };
        AddFaustHeaderRow([redacted], "NearbyHdr", cols, new[] { "Object", "Category", "Dist." });
        int i = 0;
        foreach (var (o, cat) in rows)
            AddFaustCellRow([redacted], $"Nearby{i++}", cols,
                new[] { o.Name, [redacted](cat), $"{o.Distance:0.0}m" });
    }

    private static string [redacted](ObjCategory cat)
    {
        string color = cat switch
        {
            ObjCategory.Container => "#FFD75A",
            ObjCategory.Prison    => "#E0A0E0",
            ObjCategory.Castle    => "#C9A87A",
            ObjCategory.Other     => Theme.MutedBodyHex,
            _                     => "#9FD0FF",   // resource sub-categories
        };
        return $"<color={color}>{[redacted].Label(cat)}</color>";
    }
