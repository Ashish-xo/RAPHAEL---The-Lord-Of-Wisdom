...
using PanelManager = Raphael.UI.Framework.UniverseLib.UI.Panels.PanelManager;
...
public class UIBase
{
    ...
    // B12 (0.19): the baseline sort order SetOnTop reorders around. Normally TOP_SORTORDER so Raphael
    // floats over the game HUD. While a game menu is open AND OverlaysBehindGameMenus is on,
    // UICanvasSystemPatch lowers this to its MENU_BEHIND_BASE so that even a focus-click inside a Raphael
    // panel (which fires SetOnTop every frame via PanelManager.UpdateFocus) keeps Raphael BEHIND the menu
